...
def get_existing_s_sheet(filename, sheet_name):
    wb = openpyxl.load_workbook(filename)
    ...
    for row in range(2, sheet.max_row+1):
        name = sheet.cell(row=row, column=1).value
        code = sheet.cell(row=row, column=2).value
        s_values = [sheet.cell(row=row, column=col).value for col in range(3, sheet.max_column+1)]
        stocks[code] = {'name': name, 's_values': s_values}
    return dates, stocks, wb

def update_s_to_excel(filename, dates, stocks, window, sheet_name):
    existing_dates, existing_s_stocks, wb = get_existing_s_sheet(filename, sheet_name)
    if existing_dates is None:
        return save_all_s_to_excel(filename, dates, stocks, window, sheet_name)
        ...
        for col_offset, new_date in enumerate(new_dates):
            ...
            if date_idx >= window - 1:
                ...
                if None in window_prices:
                    ...
                    val = calc_s(window_prices, window)
            else:
                val = None
...
def save_all_s_to_excel(filename, dates, stocks, window, sheet_name):
    try:
        wb = openpyxl.load_workbook(filename)
...
def main():
    ...
    for window, sheet_name in zip([20, 60, 120], ['s20', 's60', 's120']):
        update_s_to_excel(filename, dates, stocks, window, sheet_name)
